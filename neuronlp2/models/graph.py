...
from transformers.activations import ACT2FN,gelu,gelu_new,mish
...
from transformers.models.bert.modeling_bert import BertSelfOutput,BertIntermediate,BertOutput,BertPooler,\
    BertEncoder,BERT_INPUTS_DOCSTRING,BERT_START_DOCSTRING,BertPreTrainedModel,BertModel
...
## if you want to add POS tags into your embedding part, add them as special tokens, then load the adopted BERT
def initialize_bertgraph(BERT_NAME_OR_PATH, layernorm_key=False,layernorm_value=False,input_label_graph=False, input_unlabel_graph=False,label_size=None):

    bertgconfig = BertGraphConfig.from_pretrained(BERT_NAME_OR_PATH)
    # bertgconfig = SemSynRobertaConfig.from_pretrained(BERT_NAME_OR_PATH)
    init_bert = BertModel.from_pretrained(BERT_NAME_OR_PATH)
    bertgconfig.add_graph_par(layernorm_key,layernorm_value,input_label_graph, input_unlabel_graph,label_size)
...
class BertGraphConfig(PretrainedConfig):
    ...
    def add_graph_par(self,layernorm_key,layernorm_value,input_label_graph, input_unlabel_graph,label_size):
        self.label_size = label_size
        self.layernorm_key = layernorm_key
        self.layernorm_value = layernorm_value
        self.input_unlabel_graph = input_unlabel_graph
        self.input_label_graph = input_label_graph
...
class BertGraphSelfAttention(nn.Module):
    def __init__(self, config):
        ...
        if self.input_label_graph:
            self.dp_relation_k = nn.Embedding(2*config.label_size+1,self.attention_head_size)
            self.dp_relation_v = nn.Embedding(2*config.label_size+1,self.attention_head_size)
    ...
    def reset_parameters(self):
        ...
        nn.init.xavier_uniform_(self.dp_relation_k.weight)
        nn.init.xavier_uniform_(self.dp_relation_v.weight)
        with torch.no_grad():
            self.dp_relation_k.weight[0].fill_(0)
            self.dp_relation_v.weight[0].fill_(0)
    ...
    def forward(
        ...
        if graph_arc is not None:
            dp_keys = self.dp_relation_k(graph_arc.to(key_layer.device)) # [1,18,18,64]
            if self.layernorm_key:
                dep_keys = self.layernorm_key_layer(dp_keys)
                
            dp_values = self.dp_relation_v(graph_arc.to(key_layer.device))
            if self.layernorm_value:
                dep_values = self.layernorm_value_layer(dp_values)
        ...
        if graph_arc is not None:
            attention_scores = attention_scores + self.relative_matmul_dp(query_layer, dp_keys) #query_layer:[1,12,18,64] dp_keys=[1,18,18,64]
