...
# <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<< sdp <<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<
def eval_sdp(words, postags, heads_pred, types_pred, heads, types, word_alphabet, pos_alphabet,lengths,
         punct_set=None, symbolic_root=False, symbolic_end=False, err_types=None):
    batch_size, _ = words.shape
    ucorr = 0.
    lcorr = 0.
    ...
    type_true = 0
    type_pred_num = 0
    # type_cal = np.zeros((3, type_alphabet.size()), dtype=int)  # index 0: true_1 , index 1: pred_1, index 2: pred_1 and true_1
    # ===================================================================
    for i in range(batch_size):
        ucm = 1.
        lcm = 1.
        ...
        for j in range(start, lengths[i] - end):
            ...
            if err_types is not None:
                # err_types has no symbolic_root
                err_type = err_types[i][j - start]  # print (word, pos, err_type)
            for k in range(0, lengths[i] - end):
                if heads[i, j, k] == 1:
                    total += 1
                    if heads[i, j, k] == heads_pred[i, j, k]:
                        ucorr += 1
                        if types[i, j, k] == types_pred[i, j, k]:
                            lcorr += 1
                        else:
                            lcm = 0
                    else:
                ...
                # =====================f1 ===========================
                if heads[i, j, k] and heads_pred[i, j, k]:
                    arc_tp += 1
                elif heads[i, j, k] and not heads_pred[i, j, k]:
                    arc_fn += 1
                elif not heads[i, j, k] and heads_pred[i, j, k]:
                    arc_fp += 1
                else:
                    arc_tn += 1
                ...
                if heads[i,j,k]:
                    type_true +=1
                    if types[i, j, k] == types_pred[i, j, k] and heads_pred[i,j,k]:
                        type_match +=1
                if heads_pred[i,j,k]:
                    type_pred_num +=1
                # ===========================================================
            ...
            if heads[i, j, 0] == 1:
                total_root += 1
                corr_root += 1 if heads_pred[i, j, 0] == 1 else 0
    ...
    return (ucorr, lcorr, total, ucomplete_match, lcomplete_match), (ucorr_nopunc, lcorr_nopunc, total_nopunc, ucomplete_match_nopunc, lcomplete_match_nopunc), (ucorr_err, lcorr_err, total_err), (
    ucorr_err_nopunc, lcorr_err_nopunc, total_err_nopunc), (corr_root, total_root), batch_size, (arc_tp,arc_fp,arc_tn,arc_fn), (type_match,type_true,type_pred_num)
